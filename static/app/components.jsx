/* FrontierOS — shared UI components */
...
function Sidebar({ page, setPage, cats, activeCats, toggleCat }) {
  ...
  const profile = ['LLM Agents', 'RAG', 'ML Systems', 'Computer Vision'];

  // Try to read user from localStorage (set by AuthProvider in v2pages.jsx)
  let userInfo = null;
  try { userInfo = JSON.parse(localStorage.getItem('rr_user') || 'null'); } catch (_) {}
  const isAdmin = userInfo?.is_admin;

  // Items only shown to admins
  const adminItems = new Set(['admin', 'orchestrator']);

  return (
    <aside className="sidebar">
      <div className="sb-brand"><Mark /> FrontierOS</div>

      <nav className="sb-section">
        {NAV.filter(n => !adminItems.has(n.id) || isAdmin).map(n => (
      ...
      <div className="sb-profile">
        <div className="av">{userInfo ? (userInfo.full_name || userInfo.email || '?').slice(0,2).toUpperCase() : 'RR'}</div>
        <div className="who">
          <b>{userInfo?.full_name || userInfo?.email || 'Guest'}</b>
          <span>{userInfo ? (isAdmin ? 'Admin · Lab' : 'Researcher · Free') : 'Not signed in'}</span>
        </div>
      </div>
    </aside>
  );
...
function PaperCard({ paper, onOpen, onAction }) {
  ...
  return (
    <div className="card paper-card" onClick={() => onOpen(paper)}>
      ...
      {paper.methods && paper.methods.length > 0 && <div className="pc-tax"><b>Methods:</b> {paper.methods.join(', ')}</div>}
      {paper.datasets && paper.datasets.length > 0 && <div className="pc-tax"><b>Datasets:</b> {paper.datasets.join(', ')}</div>}
      <div className="pc-actions" onClick={e => e.stopPropagation()}>
        <button className="btn btn-accent btn-sm" onClick={() => onAction('open', paper)}><Icon name="doc" size={14} /> Deep Dive</button>
        <button className="btn btn-sm" onClick={() => onAction('code', paper)}><Icon name="code" size={14} /> Generate Code</button>
        <button className="btn btn-sm" onClick={() => onAction('save', paper)}><Icon name="bookmark" size={14} /> Save</button>
        <button className="btn btn-sm btn-ghost" onClick={() => onAction('more', paper)}>More like this</button>
        <button className="btn btn-sm btn-ghost" onClick={() => onAction('ignore', paper)}>Ignore</button>
        {paper.arxiv_url && (
          <a className="btn btn-sm btn-ghost" href={paper.arxiv_url} target="_blank" rel="noopener noreferrer"
             onClick={() => onAction('arxiv', paper)}>arXiv ↗</a>
        )}
        {paper.pdf_url && (
          <a className="btn btn-sm btn-ghost" href={paper.pdf_url} target="_blank" rel="noopener noreferrer"
             onClick={() => onAction('pdf', paper)}>PDF ↗</a>
        )}
      </div>
    </div>
  );
